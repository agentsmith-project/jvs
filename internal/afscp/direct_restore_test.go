...
func TestDirectRestoreSuccessLeavesOldHomeBackupForOutOfBandCleanup(t *testing.T) {
	...
	cleanupMetadata, err := readDirectCleanupMetadata(newDirectLayout(ResolvedSelector{ControlRoot: controlRoot, Home: home}), cleanupMarkers[0])
	require.NoError(t, err)
	assert.Equal(t, save.SavePointID, cleanupMetadata.SavePointID)
	assert.Equal(t, backupNames[0], cleanupMetadata.BackupHomeName)
