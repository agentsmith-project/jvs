...
func TestSeparatedLifecycleUnsupportedCommandsFailClosedJSON(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, "E_EXTERNAL_LIFECYCLE_UNSUPPORTED", env.Error.Code)
...
func TestSeparatedLifecycleUnsupportedHumanOutputSaysNoFilesChanged(t *testing.T) {
	...
	assert.Contains(t, stderr, "control data is outside the folder")
	assert.Contains(t, stderr, "No files changed")
	assert.NotContains(t, stderr, "separated-control")
...
func TestSeparatedLifecycleReadonlyWorkspaceCommandsStillWork(t *testing.T) {
	...
	assertExternalControlDataShape(t, listData, controlRoot, payloadRoot, "main")
	...
	assertExternalControlDataShape(t, pathData, controlRoot, payloadRoot, "main")
