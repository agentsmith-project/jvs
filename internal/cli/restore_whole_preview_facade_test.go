...
func assertRestoreExpectedPreviewTransferDestination(t *testing.T, data map[string]any, repoRoot, sourceID, publishedDestination string) {
	...
	require.Equal(t, "save_point:"+sourceID, primary["source_path"])
	require.Equal(t, "temporary_folder", primary["destination_role"])
	require.Equal(t, "temporary_folder", primary["materialization_destination"])
	require.Equal(t, "control_data", primary["capability_probe_path"])
...
func assertRestoreRunSourceValidationTransfer(t *testing.T, record map[string]any, repoRoot, sourceID, publishedDestination string) {
	...
	require.Equal(t, "save_point_content", record["source_role"])
	require.Equal(t, "save_point:"+sourceID, record["source_path"])
	require.Equal(t, "temporary_folder", record["destination_role"])
	require.Equal(t, "temporary_folder", record["materialization_destination"])
	require.Equal(t, "control_data", record["capability_probe_path"])
...
func assertRestoreFinalRunTransferRecord(t *testing.T, primary map[string]any, repoRoot, sourceID, publishedDestination, transferID, _ string) {
	t.Helper()
