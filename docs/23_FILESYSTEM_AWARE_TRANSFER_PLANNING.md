...
用户在一个 workspace 中运行 `jvs save -m "baseline"`。JVS 应按 workspace managed files 到 save point staging/payload 的真实路径规划复制方式。若 workspace 和 JVS storage 位于不同挂载点，不能因为 repo root 看起来支持某种 engine 就误判本次 save 可以快。

边界：

- `materialization_destination`: 实际写入或 clone 的 save point staging/payload path。
...
用户运行 `jvs workspace new ../experiment --from <save>`，目标 folder 可能在 repo 外部、另一个磁盘、另一个挂载点或同一挂载点。JVS 应按 save point payload 到新 workspace materialization path 的路径组合规划复制方式。
...
- `materialization_destination`: restore run 实际写入的 staging/tmp/payload path，或实现中真正 copy 到的 workspace target path。
...
用户运行 `jvs view <save>` 或 `jvs view <save> <path>`。JVS 创建只读 view 时，应按 save point payload/path 到 view materialization path 的真实组合规划。不要因为 repo root 支持快路径，就假设临时 view 目录也支持。
...
- 开发若只拿最终 folder 做 pair probe，容易忽略真正写入的是 staging/tmp/payload path，也容易在 final folder 不存在时做错误判断。
...
| `source_role` | `workspace_content`、`save_point_payload`、`restore_target`、`recovery_backup_payload` 等 |
| `source_path` | 本次真实 source path |
| `destination_role` | `save_point_staging`、`workspace_folder`、`restore_staging`、`view_directory`、`backup_restore_target` 等 |
| `materialization_destination` | 实际写入、clone 或普通复制的 staging/tmp/payload path；没有 copy 点时应为空或不存在 |
...
- Save: workspace content -> save point staging/payload。
- Workspace new: save point payload -> workspace materialization path，published destination 是 explicit target folder。
...
- View: save point payload/path -> view materialization path，按实际 view destination 规划。
    ...
    "transfers": [
      {
        ...
        "source_path": "/abs/repo/.jvs/...",
        "destination_role": "workspace_folder",
        "materialization_destination": "/abs/experiment/.jvs-tmp/materialize",
...
- [ ] Workspace new 使用 save point payload -> workspace materialization boundary 规划。
