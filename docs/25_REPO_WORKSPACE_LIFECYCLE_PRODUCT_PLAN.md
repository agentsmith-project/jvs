...
**Status:** implemented design record for current repo/workspace lifecycle behavior; active clean redesign, non-release-facing, not part of the v0 public contract. The release-facing source of truth is `docs/02_CLI_SPEC.md` and `docs/user/`; later lifecycle refinements stay here until promoted.

**中文名:** Repo 与 workspace 生命周期操作。

本文面向产品、工程、安全 UX 和 QA，记录 JVS repo/workspace 生命周期操作的产品口径和验收边界。它只讨论文档和行为合同，不包含实现代码；已经 release-facing 的命令以 CLI spec 和 user docs 为准，后续 refinements 留在本文直到 promoted。

Scope note: 本 handoff 描述 embedded/current lifecycle model：active `.jvs` 位于 project folder，main workspace folder 等于 repo root，main workspace registry `RealPath` 等于 repo root。不要把本文的 folder-based repo model 套到 External Control Root handoff；workspace folder 外部 control data location 的 repo 由 `docs/26_EXTERNAL_CONTROL_METADATA_PRODUCT_PLAN.md` 覆盖，其 Phase 1 lifecycle commands 必须 unsupported fail closed，Phase 2 需要重写 external-control lifecycle matrix。
...
- 不在 move/rename/delete/detach 中修改用户文件内容、save point descriptors、content storage、audit/provenance 或历史语义。
...
- 不由本文直接覆盖 release-facing CLI spec、user docs 或 public contract；若行为需要发布，必须先提升到 release-facing docs。
...
- save point history、content storage、audit/provenance 不因 lifecycle move/rename/delete/detach 被重写。
...
Scope note: 本节的 `main workspace folder == repo root`、main workspace registry `RealPath == repo root` 合同只适用于 embedded/current lifecycle model。External Control Root handoff 由 `docs/26_EXTERNAL_CONTROL_METADATA_PRODUCT_PLAN.md` 覆盖；workspace folder 外部 control data location 的 Phase 1 lifecycle commands 必须 unsupported fail closed，Phase 2 再重写 external-control lifecycle matrix。
...
- `workspace delete` 不能删除 save point storage、save point descriptors、repo history 或 content storage。
...
| `workspace new` | 继续直接创建 | 创建新目标，不移动或删除已有真实对象；当前公开合同没有 `workspace new --dry-run` |
...
- save point storage、save point descriptors、content storage 和 repo history 不由 delete 清理，留给 cleanup 的 reviewed flow。
...
- archive path 的 `<operation-id>` 必须来自 run journal 的 `operation_id`，不得从 `plan_id` 派生。plan_id 只是 preview/run handle；operation_id 是 run journal identity，二者必须不同；所有 `archive_path` 字段都必须沿用 journal identity。
...
`workspace reconnect` 在新产品心智里不是主命令。它最多可以存在为内部 repair primitive 或 troubleshooting 下由明确错误提示引导的窄动作，不应作为 later design record 的标题、日常命令入口或用户必须先理解的概念。
...
- 记录 publish gate：发布前必须验证 release-facing docs 不出现 `workspace reconnect` 或 `remove` 旧心智；本文仍是 non-release-facing design record。
...
- [ ] release-facing CLI spec/user docs 只吸收已发布行为；release-facing docs clean 是 publish gate，不由本文单独声明发布。
