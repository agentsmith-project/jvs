...
- Status: PASS
- Tag: `v0.4.8`
- Final tagged commit: `589c4d7ccaf20813f02b9d8017a9909b701be8b9`
- Commit message: `Prepare v0.4.8 release`
- Tag object: `c00188b4fde18d704c0c349e4d1ab59e9555feac`
...
- Release listing: GitHub release list shows `v0.4.8` as Latest.
- Scope: final release evidence for external control root restore-run stable
  closed loop; recovery status/doctor/clone state interpretation consistency;
  global malformed guard for recovery resume and rollback; separated clone
  source recovery recheck before target publication; malformed external restore
  state user story E2E; and release-binary-smoke restore+clone coverage.
- Scope/baseline for `v0.4.8` final: external control root restore-run stable closed loop; recovery status/doctor/clone state interpretation consistency; global malformed guard for recovery resume and rollback; separated clone source recovery recheck before target publication; malformed external restore state user story E2E; release-binary-smoke restore+clone coverage.
...
tag-gated publication, by workflow run `25369519260`.
...
| Coverage | `make test-cover` | PASS; `69.2% >= 60%` |
...
| Release binary smoke | `make release-binary-smoke` using `^TestStorySeparated(Restore|Clone)` | PASS with restore and clone stories against the release-built Linux artifact |
...
| Security scan | GitHub Actions Security Scan job | PASS in tag workflow run `25369519260` |
| Release toolchain smoke | GitHub Actions Release Toolchain Smoke job | PASS in tag workflow run `25369519260` |
| Release publication | GitHub Actions Release job | PASS in tag workflow run `25369519260` |
| DCO | GitHub Actions DCO job | SKIPPED in tag workflow run `25369519260` |
...
- Representative repo coverage: release-gate suite coverage for save point
  ...
  behavior, runtime repair path, external control root restore-run stable
  closed loop, recovery status/doctor/clone state interpretation consistency,
  ...
  user story E2E, story-e2e gate coverage, and release-binary smoke restore and
  clone stories.
- External control root restore-run evidence:
  `TestStorySeparatedRestoreRunStableStateDoctorStrictAndClone` covers restore
  preview/run through a stable closed loop where recovery status clears, source
  doctor strict reports healthy, and separated clone can proceed from the
  restored source.
- Recovery status, doctor, and clone consistency evidence:
  `TestStorySeparatedRestoreStalePreviewRecoveryCommandCleanupAndCloneConsistency`
  covers stale restore state reporting, doctor strict unhealthy evidence, clone
  blocking with the same recommended cleanup command, cleanup through the
  published command, and successful clone after cleanup.
- Malformed external restore state user story E2E evidence:
  `TestStorySeparatedRestoreMalformedStateBlocksRecoveryDoctorAndCloneWithoutControlPlaneLeak`
  covers recovery status failing closed, doctor strict reporting the
  recovery_state check, clone blocking, clean-CWD selector hints, and public
  error text for malformed external restore state.
- Resume/rollback malformed guard evidence:
  `TestSeparatedRecoveryResumeRollbackRejectMalformedGlobalStateBeforeMutation`
  covers `jvs recovery resume` and `jvs recovery rollback` rejecting malformed
  restore or recovery state before mutation and pointing to
  `doctor --strict --json`.
- Separated clone source recovery recheck evidence:
  `TestSeparatedCloneRejectsSourceStateIntroducedBeforePublish` covers
  separated clone revalidating source recovery before target publication and
  blocking if active or malformed source recovery appears after staging.
- Release binary smoke evidence: `make release-binary-smoke` ran
  `^TestStorySeparated(Restore|Clone)` with `JVS_BINARY_UNDER_TEST` pointing at
  the release-built Linux artifact, so restore and clone stories exercised the
  binary that was published.
...
- Restore drill, lifecycle recovery drill, runtime repair evidence,
  ...
  story-e2e gate coverage, and release-binary restore/clone smoke: PASS via
  release-gate conformance, regression, and release-binary-smoke coverage.
...
- Release gate includes `make release-build`, matching the release job's five
  ...
  Windows x86_64.
- Release gate also includes `make release-binary-smoke`, which ran the
...
- Pre-upload verification: Release job verified artifacts and signatures before upload
  with the release workflow certificate identity and OIDC issuer.
...
- Release listing: At publication, GitHub release list showed `v0.4.7` as Latest.
