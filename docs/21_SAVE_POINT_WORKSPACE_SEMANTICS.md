...
避免在文件已经编辑后继续说 `Files copied from`，因为它会暗示当前文件仍与来源完全一致。干净状态可以说 `Files match save point A`；编辑后应说 `Files were last restored from A` 或 `Started from A`，并同时展示 `Unsaved changes: yes`。
...
保存点发布后，内容和创建事实不可重写。后续用户 metadata，例如 `label`、`keep`、展示用 note 或 key-value，可以作为独立可变 metadata 或索引表达，不能通过改写保存点创建事实来实现。
...
- 默认人类输出使用精确来源文案：`Files match save point`、`Files were last restored from`、`Started from`、`Restored paths`。
...
2. 解析目标 workspace 和真实文件夹路径。
3. 获取 workspace operation lock。
4. 重新扫描真实文件系统和 managed/ignored/unmanaged set。
5. 如果目标 workspace 有 `unsaved changes`，默认拒绝。
6. 如果用户显式选择 `--save-first`，先保存目标 workspace 当前未保存内容，再继续；后续 restore 时的最新保存点就是这次刚创建的新保存点。
7. 如果用户显式选择 `--discard-unsaved`，丢弃未保存内容，再继续。
8. 用 source 保存点 managed files 完整替换目标 workspace managed files。
9. 替换后，目标 managed files 必须完整匹配 source save point。
10. 目标中 source 不存在的 managed files 会被删除。
11. 目标 ignored/unmanaged files 默认保留；如果操作会覆盖/删除它们，必须拒绝，除非用户显式传入高摩擦 flag。
12. 设置目标 `content_source=<save>`。
13. 清空目标 `path_sources`。
14. 设置目标 `unsaved changes=false`。
15. 保持目标 `history_head` 不变。
16. 操作后重新检查目标文件没有被并发改写；发现变化则失败并进入恢复计划。
17. 释放 workspace operation lock。
...
Managed files missing from A will be deleted.
...
1. `<save>` 必须解析为唯一保存点 ID；不得解析 label。
2. `<path>` 必须是 workspace-relative path。
3. `<path>` 可以是文件或目录。
4. 默认只恢复当前 workspace。
5. 只替换指定 path 下的 managed files。
6. 不触碰 path 外的 managed files。
7. 不触碰 ignored/unmanaged files，除非用户显式传入高摩擦 flag。
8. 如果 source save point 中不存在 `<path>`，默认失败并说明没有修改任何文件。
9. 如果目标 `<path>` 下存在会被覆盖或删除的 unsaved managed content，默认拒绝。
10. 如果目标 `<path>` 已被误删且 source save point 中存在该 path，可以直接恢复；这是创建缺失内容，不需要恢复整个 workspace。
11. dirty guard 只检查和保护相关 path；path 外的 unsaved changes 不阻止 single-path restore。
12. 如果用户传入 `--save-first`，先保存整个 workspace 当前内容，再恢复该 path。
13. 如果用户传入 `--discard-unsaved`，只丢弃该 path 的未保存改动，再恢复该 path；path 外未保存改动保留。
14. 如果 `<path>` 是目录，source 中不存在的目标 managed files 会在该目录内删除，以便该目录 managed files 匹配 source。
15. 设置或更新 `path_sources[<path>]=<save>`。
16. 不移动 `history_head`。
17. 不改变 whole-workspace `content_source`，除非实现选择在 JSON 中表达 mixed source；默认人类输出用 `Restored paths` 表达。
18. 操作成功后，相关 path 的 dirty 状态清除；如果 path 外还有未保存改动，workspace 仍显示 `Unsaved changes: yes`。
19. 后续 `save E` 线性追加到当前 `history_head` 后面，并记录 `E.restored_paths`。
...
1. `<save>` 必须解析为唯一保存点 ID；不得解析 label。
2. `<name>` 必须是合法且未使用的 workspace name。
3. 在配置的 workspace 位置创建新的真实文件夹。
4. 把 source 保存点 managed files 复制/准备到该真实文件夹。
5. 注册 workspace metadata。
6. 设置新 workspace `history_head=null`。
7. 设置新 workspace `content_source=<save>`。
8. 设置新 workspace `started_from=<save>`。
9. 设置新 workspace `path_sources=empty`。
10. 设置 `unsaved changes=false`。
11. 不影响原 workspace。
12. 不改写 source 保存点。
13. 不把 source 保存点或 source workspace 的 parent chain 作为新 workspace 的历史线。
14. 输出中必须先显示新 workspace 的真实 `Folder`，再显示 workspace name。
...
9. 将 source 保存点 managed files 复制到 target workspace，使 target managed files 完整匹配 source。
10. 在 target workspace 创建新保存点 `T`。
11. 设置 `T.parent=<T 保存前的 target history_head>`。
12. 设置 `T.restored_from=<source save>`。
13. 将 target workspace `history_head` 和 `content_source` 移到 `T`。
14. 清空 target `path_sources`。
15. 设置 target `unsaved changes=false`。
16. source workspace 不变。
17. source 保存点不变。
18. 操作后重新检查 target 没有被并发改写；发现变化则失败并进入恢复计划。
19. 操作 resolved 后释放 source active-operation protection。
20. 释放 target workspace operation lock。
...
6. 必须重算 managed files 的 unsaved 状态；如果存在 unsaved managed changes，默认拒绝，并要求用户选择：`--save-first`、`--discard-unsaved` 或取消。
7. 如果真实 workspace folder 内存在 ignored/unmanaged files，默认拒绝删除真实文件夹和 workspace metadata；它不做“删除 metadata 但留下 folder”的半删除。用户必须先移走这些文件，或显式传入 `--delete-unmanaged` / 等价高摩擦 flag 才能删除。
8. 成功删除时，删除整个真实 workspace folder 和 workspace metadata。
9. 不删除任何保存点。
10. 输出必须提示：该 workspace 的未 keep 保存点未来可能成为 cleanup candidates。
11. 如果删除失败，不得留下 unregistered folder 或 orphan metadata；无法事务化时必须进入 recovery plan，并输出 recovery plan ID 或明确的 retry/rollback 语义。
12. 删除完成后释放 operation lock。
...
- shared/protected bytes，说明因为去重、共享块、keep、live workspace、active operation 或 recovery plan 而不能释放的字节。
...
V1 cleanup 默认不清理活跃 workspace 的旧 history chain。也就是说，live workspace 的 `history_head` 及 parent chain 默认都受保护。V1 cleanup 主要清理 removed workspace history、orphaned provenance、active view/recovery 释放后的 unprotected candidates。活跃 workspace history pruning 进入 Future retention policy；必须设计 tombstone、restore error 和 history display 语义后才能启用。
...
6. 保护 live workspace 所需保存点：
   ...
   - active view 引用的保存点
   - active operations 和 recovery plans 引用的保存点
...
jvs label add A17c92 best
jvs history --label best
jvs keep A17c92
```
...
- 替换或移动 best 必须由用户显式执行，例如先 `label remove <save> best` 再 `label add <save> best`。
- 用户必须选择具体 save ID。
- 重要结果必须 `keep`。
- metadata 中可识别为数值的值必须按数值排序，而不是按字符串排序，例如 `acc=0.94` 应排在 `acc=0.937` 之后。
- `save` 创建时写入的 metrics 属于保存点创建时事实；后续可编辑 metadata/note/label 必须走独立 metadata/audit，不得改写创建事实。
...
| `active_operation` | 未安全完成的 operation 或 recovery plan 正在引用该保存点。 |
...
| `unprotected_old_save` | Future retention policy 可能标记为可删除；MVP/V1 对 active workspace history 保持保守。 |
...
- active workspace 的 history chain 在 V1 默认不被 cleanup 删除，因此 active workspace 的普通 `history` 不应出现断链。
...
- `restore <save>` full restore 必须全事务化；如果 MVP backend 无法全事务化，MVP 必须包含最小 `recovery status` / retry / rollback 能力，否则不得声称 full restore 已闭环。
...
V1 补齐多 workspace、agent 实验闭环、Data/ML metadata 和 cleanup 管理：

- `workspace new <name> --from <save>`，且新 workspace `history_head=null`。
- workspace list/path/remove，且 remove 有 preview/confirm、dirty guard、unmanaged protection 和 recovery plan。
- `restore <save> --to <workspace> --save`
- 完整 `label` 管理
- 完整 `keep` 管理
...
- active view / active operation / recovery plan cleanup protection
...
- AI agent golden scenario：
  `save A -> agent bad edits -> restore A --discard-unsaved -> workspace new exp --from A -> agent success -> save E -> restore E --to main --save`

### Future

Future 可以包括：

- active workspace history pruning retention policy
- pruning 后 tombstone/history/view/restore error 完整语义
...
```text
A17c92  "train run 42"  acc=0.937  loss=0.21  seed=1234  labels=run,best  keep=yes
```
...
- [ ] 数值 metadata 按数值排序；后续可编辑 metadata 通过 audit 表达，不改写保存点创建事实。
...
- [ ] V1 默认不清 active workspace history chain。
...
- [ ] MVP full restore 要么全事务化，要么包含最小 recovery/status 能力。
...
- [ ] 自动化测试覆盖 V1 默认不 prune active workspace history chain。
