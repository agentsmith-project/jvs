...
但高级/operator 调用方可以用 `--control-root <path> --workspace main` 把
...
| External control root | JVS control data 位于 workspace folder 外部的可信 control root | 当前合同必须显式指定 control root + main workspace |
...
conformance 必须以 `E_WORKSPACE_CONTROL_MARKER_PRESENT` fail closed；该 path 不能作为
JVS authority。
...
- 该 `.jvs` path 必须以 `E_WORKSPACE_CONTROL_MARKER_PRESENT` fail closed，不能读取、
...
当前 conformance CLI selector 固定为 `--control-root <path> --workspace main`。
当前 external control root 合同只发布 main workspace；调用方必须能在干净 CWD
下显式指定 external control root 和 main workspace，稳定操作目标 workspace，
不受 workspace folder locator、另一个 repo 的 CWD 或 ambient discovery 影响。
...
```bash
jvs --control-root <control-root> --workspace main status --json
jvs --control-root <control-root> --workspace main doctor --strict --json
jvs --control-root <control-root> --workspace main save -m "message" --json
```

未来可以新增 config、environment 或 library API 等等价入口，例如：

```bash
JVS_CONTROL_ROOT=<path> JVS_WORKSPACE=main jvs status --json
```

这些 future/API 入口不是当前 conformance selector。conformance suite 和 handoff
examples 必须使用固定 selector，不能保留含糊语义：

- `--control-root <path> --workspace main` 是当前 external control root
  conformance selector。
- 如果未来扩展 `--repo`，必须明确 `--repo <path>` 在 external control root
  工作流中只能指向 control root，并要求同时提供 `--workspace main` 或等价
  main workspace selector；它不是当前 conformance selector。
...
- discovery 不读取 workspace folder 下的 `.jvs` 作为 authority。
...
`jvs --control-root <path> --workspace main doctor --strict --json`，由 doctor
...
| `recovery_state` | pending、stale、active 或 malformed restore/recovery state、runnable restore plan、recovery identity mismatch；stale restore preview 通过 `restore discard <restore-plan-id>` 清理；successful restore run leaves no active recovery，completed restore plan residue is non-blocking |
...
| --- | --- | --- |
| `root_overlap` | same root fixture | `E_CONTROL_WORKSPACE_OVERLAP` |
| `root_overlap` | workspace-inside-control fixture | `E_WORKSPACE_INSIDE_CONTROL` |
| `root_overlap` | control-inside-workspace fixture | `E_CONTROL_INSIDE_WORKSPACE` |
| `workspace_control_marker` | workspace root-level `.jvs` file, directory, or symlink | `E_WORKSPACE_CONTROL_MARKER_PRESENT` |
...
| `recovery_state` | pending、stale、active 或 malformed restore/recovery state blocks a new mutation；completed restore plan residue is non-blocking | `E_RECOVERY_BLOCKING` |
...
| 错误码 | 场景 | 行为 |
| --- | --- | --- |
| `E_CONTROL_WORKSPACE_OVERLAP` | control root 与 workspace folder 是同一路径、同一物理 identity，或重叠关系无法进一步分类 | fail closed before mutation |
| `E_WORKSPACE_INSIDE_CONTROL` | workspace folder 位于 control root 内部 | fail closed before mutation |
| `E_CONTROL_INSIDE_WORKSPACE` | control root 位于 workspace folder 内部 | fail closed before mutation |
...
| `E_WORKSPACE_MISSING` | 已登记/被选择 workspace 的 folder 不存在或不可达 | fail closed |
...
| `E_WORKSPACE_CONTROL_MARKER_PRESENT` | 当前 strict external profile 下 workspace folder 出现 root-level `.jvs` path；file、directory 或 symlink 都算 present | fail closed；不能读取为 authority |
...
| `E_ATOMIC_PUBLISH_BLOCKED` | clone staging 无法 no-overwrite/atomic publish 到 final target，或 target workspace folder publish 后 control publish 失败 | fail closed；source unchanged；target 不作为 active JVS repo；如果 rollback 不能安全移除 target folder 或 target control data root，target folder or target control data root may remain at the target path or be moved to a hidden quarantine；in either case, inspect/remove manually；只有 moved to quarantine 后才提示 `target folder was quarantined at ...; inspect and remove it manually` 或 `target control root was quarantined at ...; inspect and remove it manually` |
...
| `E_EXTERNAL_LIFECYCLE_UNSUPPORTED` | external control root 上调用当前未实现的 move/rename/delete/detach lifecycle 命令 | stable unsupported fail closed |
...
| `E_RECOVERY_BLOCKING` | pending、stale、active 或 malformed restore/recovery state，或 runnable restore plan 要求先 run/discard/resume/rollback/diagnose，不能开始新的 mutation；stale restore preview 的 public entry 是 `restore discard <restore-plan-id>`；successful restore run leaves no active recovery，completed restore plan residue is non-blocking | fail closed；如果存在安全 recovery 下一步，用 `error.hint` 或 human output 展示 |
...
- workspace folder 中如存在 root-level `.jvs` path，无论它是 file、directory
  ...
  external profile 必须以 `E_WORKSPACE_CONTROL_MARKER_PRESENT` fail closed；未来更多
...
- workspace folder 下 root-level `.jvs` file、directory 或 symlink 仍必须以
  `E_WORKSPACE_CONTROL_MARKER_PRESENT` fail closed，因为它会混淆 control authority。
...
- workspace folder 的 root-level `.jvs` path，file、directory 或 symlink 都算
  present；strict external profile 下应先因 `E_WORKSPACE_CONTROL_MARKER_PRESENT` 失败。
...
- successful restore run leaves no active recovery；completed restore plan
  residue is non-blocking for `recovery status`、`doctor --strict` 和
  `repo clone`。
- stale restore preview 必须有 public cleanup entry：
  `restore discard <restore-plan-id>`；平台不得删除 private restore plan file。
...
- 平台调用方不得读取或删除 private control data files 来判断是否可用；pending、
  stale、active 或 malformed restore/recovery state 必须通过 `recovery status`、
  `doctor --strict` 或被阻塞命令的 public diagnostic 入口暴露。
...
- target publish 必须是 no-overwrite/atomic publish 语义；发布被阻塞或 target
  workspace folder publish 后 control publish 失败时，以
  `E_ATOMIC_PUBLISH_BLOCKED` fail closed，source unchanged，target 不作为 active
  JVS repo。若 rollback 不能安全移除 target folder 或 target control data root，
  target folder or target control data root may remain at the target path or be
  moved to a hidden quarantine；in either case, inspect/remove manually。只有
  moved to quarantine 后才提示
...
- 错误码使用 external control root 口径的 `E_EXTERNAL_LIFECYCLE_UNSUPPORTED`。
- 不移动 control root。
...
- 固定 conformance CLI selector 为 `--control-root <path> --workspace main`；
...
- 实现固定 conformance selector：`--control-root <path> --workspace main`。
...
- pending、stale、active 或 malformed restore/recovery state 阻塞新 mutation，并通过
  `error.hint`、human output 或 doctor `checks[]` 给出安全诊断/恢复入口；
  stale preview 推荐 `restore discard <restore-plan-id>`，runnable preview 推荐
  `restore --run <restore-plan-id>`；
  普通错误响应仍保持 `data:null`。completed restore plan residue is
  non-blocking。
...
| Root same path | `C == W` | `E_CONTROL_WORKSPACE_OVERLAP`；无 mutation | 默认 CI |
| Workspace inside control | `W=C/workspace` | `E_WORKSPACE_INSIDE_CONTROL`；无 mutation | 默认 CI |
| Control inside workspace | `C=W/.control` | `E_CONTROL_INSIDE_WORKSPACE`；无 mutation | 默认 CI |
...
| Workspace marker present | workspace folder 下 root-level `.jvs` file、directory 或 symlink | strict external profile `E_WORKSPACE_CONTROL_MARKER_PRESENT`；doctor check `workspace_control_marker` failed；不得读取为 authority | 默认 CI |
...
| Missing registered workspace | registry 中被选择 workspace 指向的 folder missing | `E_WORKSPACE_MISSING` | 默认 CI |
...
| Recovery blocking | control root 有 pending、stale、active 或 malformed restore/recovery state，或 runnable restore plan；completed restore plan residue is non-blocking | 新 mutation `E_RECOVERY_BLOCKING`，并给出 public diagnostic 入口；stale preview 推荐 `restore discard <restore-plan-id>` | 默认 CI |
...
| Clone target same path | target `TC == T` | clone fail closed before publish；`E_CONTROL_WORKSPACE_OVERLAP` | 默认 CI |
| Clone target workspace inside control | target `T=TC/workspace` | clone fail closed before publish；`E_WORKSPACE_INSIDE_CONTROL` | 默认 CI |
| Clone target control inside workspace | target `TC=T/.control` | clone fail closed before publish；`E_CONTROL_INSIDE_WORKSPACE` | 默认 CI |
| Clone target occupied | target control/workspace root non-empty 或需 merge/overwrite/adopt | `E_TARGET_ROOT_OCCUPIED`；source unchanged；no active target repo | 默认 CI |
| Clone source dirty | source workspace dirty 或 writer race fake | `E_SOURCE_DIRTY`；source unchanged；no active target repo | 默认 CI |
...
| Clone atomic publish blocked | staging 后 final target 被创建、no-replace rename 失败，或 target folder publish 后 control publish 失败 | `E_ATOMIC_PUBLISH_BLOCKED`；source unchanged；target 不是 active JVS repo；无法安全移除 target folder 或 target control data root 时，target folder or target control data root may remain at the target path or be moved to a hidden quarantine；in either case, inspect/remove manually；只有 moved to quarantine 后才提示 `target folder was quarantined at ...; inspect and remove it manually` 或 `target control root was quarantined at ...; inspect and remove it manually` | 默认 CI |
...
| Lifecycle unsupported | external control root 上调用 move/rename/delete/detach | `E_EXTERNAL_LIFECYCLE_UNSUPPORTED`；No files changed | 默认 CI |
...
| 风险 | 影响 | 缓解 |
| --- | --- | --- |
| `--repo` 语义含糊 | 平台可能把 workspace folder 当 control root，或受 CWD 影响 | 当前 conformance selector 固定为 `--control-root <path> --workspace main`；`--repo` 只能作为未来/兼容扩展且只能指 control root |
| workspace `.jvs` 被误信任 | 不可信 workload 可劫持 repo identity 或 workspace | strict external profile 以 `E_WORKSPACE_CONTROL_MARKER_PRESENT` fail closed；doctor check `workspace_control_marker` 固定映射该错误码 |
...
- [ ] explicit targeting tests：clean CWD、另一个 repo CWD、missing explicit
  target、固定 `--control-root <path> --workspace main` selector。
...
- [ ] workspace marker tests：workspace folder root-level `.jvs` absent，以及
  ...
  `workspace_control_marker` failed 且 error code 为 `E_WORKSPACE_CONTROL_MARKER_PRESENT`。
...
- [ ] external init control root 只接受 missing 或 empty directory；
  ...
  workspace root-level `.jvs` 以 `E_WORKSPACE_CONTROL_MARKER_PRESENT` fail closed。
...
- [ ] dirty source、writer race、target root overlap、target root occupied 在
  publish 前 fail closed；atomic publish blocked 或 target folder publish 后
  control publish 失败使用稳定错误码，source unchanged，target 不作为 active JVS
  repo；rollback 无法安全移除 target folder 或 target control data root 时，target
  folder or target control data root may remain at the target path or be moved
  to a hidden quarantine。
